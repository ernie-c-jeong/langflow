...
class PolarisAIDataInsightComponent(Component):
    ...
    description: str = (
        "Polaris AI DataInsight is a document parser that extracts document elements (text, images, complex tables, charts, etc.)"
        " from various file formats into structured JSON, making them easy to integrate into RAG systems."
    )
    ...
    def extract_document(self) -> Data:
        ...
        if not self.resources_dir:
            ...
            raise ValueError("Resources directory is required.")
        ...
        try:
            ...
            self.status = f"Error loading document: {e}"
            raise e
